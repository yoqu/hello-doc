                ...
                <AutoComplete class="auto-search" placeholder="搜索" icon="ios-search" @on-search="handleSearch">
                    <div class="demo-auto-complete-item" v-for="item in searchList" v-on:click="handleCheckMenu(item)">
                        <div class="demo-auto-complete-group" v-if="item.name !='...'">
                            <span class="search-result-title" v-html="splitTitle(item.title)"></span>-<span
...
<script>
    ...
    import {fixBaseUrl, getBasicDefinition} from "@/api/doc";
    export default {
        name: "Home",
        components: {
            NavMenu: NavMenu
        ...
        methods: {
            ...
            checkUrl(baseUrl,callback){
                let url = fixBaseUrl(this.configData.baseUrl) + 'static/data/basic_definition.json';
                let self = this;
                $.ajax({
                    url: url,
                    type: 'get',
            ...
            handleSaveConfig() {
                ...
                if(this.configData.baseUrl!==''){
                    this.checkUrl(this.configData.baseUrl,function(result,response){
                        if(result){
