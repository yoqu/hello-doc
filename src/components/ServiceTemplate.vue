...
<template>
  <div>
    ...
    <p class="service-sub-header">服务路径：{{ serviceName }}
      <Divider type="vertical"/>
      <Button size="small" icon="ios-copy" type="success" shape="circle" class="copy-service">复制</Button>
    </p>
    <p class="service-sub-header">版本号：{{ version }}</p>
    <p class="service-sub-header">方法：{{ method }}</p>
    <div class="service-sub-header" v-if="finish">开发进度:
      <div class="progress">
        <Progress :percent="finish"></Progress>
      </div>
    </div>
    <Card style="margin-top: 10px;" v-if="headers_data.length>0">
      <p slot="title">请求Header头</p>
      <Table :columns="headersColumns" :data="headers_data"></Table>
    </Card>
...
export default {
  ...
  mounted: function () {
    ...
    if (this.$store.state.firstLoadService) {
      this.$store.commit("firstLoadService", false)
